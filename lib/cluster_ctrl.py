...
class InfoController(CommandController):
    ...
    def do_service(self, line):
        stats = common.cluster.infoStatistics(nodes=self.nodes)
        builds = common.cluster.info('build', nodes=self.nodes)
        services = common.cluster.infoServices(nodes=self.nodes)
        visible = common.cluster.getVisibility()
        ...
        self.view.infoService(stats, builds, visibility, common.cluster, **self.mods)
    ...
    def do_network(self, line):
        stats = common.cluster.infoStatistics(nodes=self.nodes)
        hosts = common.cluster.nodes

        # get current time from namespace
        ns_stats = common.cluster.infoAllNamespaceStatistics(nodes=self.nodes)
        ...
        self.view.infoNetwork(stats, hosts, common.cluster, **self.mods)

    @CommandHelp('Displays summary information for each namespace.')
    def do_namespace(self, line):
        stats = common.cluster.infoAllNamespaceStatistics(nodes=self.nodes)
        self.view.infoNamespace(stats, common.cluster, **self.mods)

    @CommandHelp('Displays summary information for Cross Datacenter'
                 , 'Replication (XDR).')
    def do_xdr(self, line):
        stats = common.cluster.infoXDRStatistics(nodes=self.nodes)
        builds = common.cluster.xdrInfo('build', nodes=self.nodes)
        xdr_enable = common.cluster.isXDREnabled(nodes=self.nodes)
        self.view.infoXDR(stats, builds, xdr_enable, common.cluster, **self.mods)

    @CommandHelp('Displays summary information for Seconday Indexes (SIndex).')
    def do_sindex(self, line):
        stats = common.cluster.infoSIndex(nodes=self.nodes)
        sindexes = {}
        ...
        self.view.infoSIndex(stats, common.cluster, **self.mods)
...
class ASInfoController(CommandController):
    ...
    def _do_default(self, line):
        ...
        if xdr:
            results = common.cluster.xdrInfo(value, nodes=nodes)
        else:
            results = common.cluster.info(value, nodes=nodes)

        self.view.asinfo(results, line_sep, common.cluster, **mods)
...
class ShowDistributionController(CommandController):
    ...
    def _do_distribution(self, histogram_name, title, unit):
        histogram = common.cluster.infoHistogram(histogram_name, nodes=self.nodes)
...
class ShowLatencyController(CommandController):
    ...
    def _do_default(self, line):
        ...
        latency = common.cluster.infoLatency(nodes=self.nodes)
        ...
        self.view.showLatency(hist_latency, common.cluster, **self.mods)
...
class ShowConfigController(CommandController):
    ...
    def do_service(self, line):
        service_configs = common.cluster.infoGetConfig(nodes=self.nodes
                                                     , stanza='service')
        ...
        self.view.showConfig("Service Configuration"
                             , service_configs
                             , common.cluster, **self.mods)

    @CommandHelp('Displays network configuration')
    def do_network(self, line):
        hb_configs = common.cluster.infoGetConfig(nodes=self.nodes
                                                , stanza='network.heartbeat')
        info_configs  = common.cluster.infoGetConfig(nodes=self.nodes
                                                   , stanza='network.info')
    ...
    def do_namespace(self, line):
        namespace_configs = common.cluster.infoGetConfig(nodes=self.nodes
                                                       , stanza='namespace')
        ...
        for ns, configs in ns_configs.iteritems():
            self.view.showConfig("%s Namespace Configuration"%(ns)
                                 , configs, common.cluster, **self.mods)

    @CommandHelp('Displays XDR configuration')
    def do_xdr(self, line):
        xdr_configs = common.cluster.infoXDRGetConfig(nodes=self.nodes)
        ...
        self.view.showConfig("XDR Configuration", xdr_filtered, common.cluster
                             , **self.mods)
...
class ShowStatisticsController(CommandController):
    ...
    def do_service(self, line):
        service_stats = common.cluster.infoStatistics(nodes=self.nodes)
        self.view.showStats("Service Statistics", service_stats, common.cluster
                            , **self.mods)

    @CommandHelp('Displays namespace statistics')
    def do_namespace(self, line):
        namespaces = common.cluster.infoNamespaces(nodes=self.nodes)
        ...
        for namespace in sorted(namespace_set):
            ns_stats = common.cluster.infoNamespaceStatistics(namespace
                                                            , nodes=self.nodes)
    ...
    def do_sets(self, line):
        sets = common.cluster.infoSetStatistics(nodes=self.nodes)
    ...
    def do_bins(self, line):
        bin_stats = common.cluster.infoBinStatistics(nodes=self.nodes)
        new_bin_stats = {}
    ...
    def do_xdr(self, line):
        xdr_stats = common.cluster.infoXDRStatistics(nodes=self.nodes)
...
class ClusterController(CommandController):
    ...
    def do_dun(self, line):
        results = common.cluster.infoDun(self.mods['line'], nodes=self.nodes)
        self.view.dun(results, common.cluster, **self.mods)

    def do_undun(self, line):
        results = common.cluster.infoUndun(self.mods['line'], nodes=self.nodes)
        self.view.dun(results, common.cluster, **self.mods)
